  ...
  describe '#auth_header' do
    ...
    before do
      allow(Settings).to receive(:services).and_return(
        [ OpenStruct.new(
            dashboard_client: OpenStruct.new(id: '<client id>', secret: '<client secret>')
          )
        ])
    end
    ...
    context 'when the access token is not expired' do
      before do
        CF::UAA::TokenCoder.stub(:decode).and_return('exp' => 1.minute.from_now.to_i)
      end
    ...
    context 'when the access token is expired' do
      before do
        CF::UAA::TokenCoder.stub(:decode).and_return('exp' => 1.minute.ago.to_i)

        Configuration.stub(:auth_server_url) { 'http://login.example.com' }
        Configuration.stub(:token_server_url) { 'http://uaa.example.com' }
        token_info = CF::UAA::TokenInfo.new(access_token: 'new_access_token', refresh_token: 'new_refresh_token', token_type: 'bearer')
