...
describe Configuration do
  ...
  before do
    Configuration.clear
    stub_request(:any, "#{Settings.cc_api_url}/info").
  ...
  describe '#auth_server_url' do
    it 'uses the cc_api_url to get the url for the auth server' do
      expect(Configuration.auth_server_url).to eql('http://login.10.244.0.34.xip.io')
      a_request(:get, "#{Settings.cc_api_url}/info").should have_been_made
    end
  end

  describe '#token_server_url' do
    it 'uses the cc_api_url to get the url for the token server' do
