...
describe 'the service lifecycle' do
  ...
  it 'provisions, binds, unbinds, deprovisions' do
    ...
    ##
    put "/v2/service_instances/#{instance_id}", {service_plan_id: 'PLAN-1', format: :json}, env
    ...
    ## Bind
    ##
    put "/v2/service_bindings/#{binding_id}", {service_instance_id: instance_id, format: :json}, env
    ...
    ## Unbind
    ##
    delete "/v2/service_bindings/#{binding_id}", {format: :json}, env
    ...
    ## Deprovision
    ##
    delete "/v2/service_instances/#{instance_id}", {format: :json}, env
    expect(response.status).to eq(204)
